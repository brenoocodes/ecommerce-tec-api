...
from src.config.login.token import logado, funcionario_logado
from datetime import datetime, timezone

@router.put('/funcionario/clientes/{id}', status_code=200)
async def alterar_cliente(db: db_dependency, response: Response, user: funcionario_logado, id: str, cliente: index.ClienteAlterar):
    if user['adm'] == False:
